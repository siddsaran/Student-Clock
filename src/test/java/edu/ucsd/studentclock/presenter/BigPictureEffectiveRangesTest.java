...
class BigPictureEffectiveRangesTest {
        ...
        @Test
        @DisplayName("Series ordering is based on deadline order, not input order")
        void seriesRespectsDeadlineOrderingNotInputOrdering() {
                ...
                Map<Assignment, LocalDate[]> ranges = BigPictureEffectiveRanges
                                .computeEffectiveRanges(List.of(later, earlier));
        ...
        @Test
        @DisplayName("Series start is not moved forward when stored start is already after previous effective end")
        ...
        @Test
        @DisplayName("Assignments without series do not chain across different courses")
        ...
        @Test
        @DisplayName("Effective end extends by the number of allowed late days")
        void effectiveEndExtendsByLateDays() {
                ...
                assertEquals(d.minusDays(2), range[0]);
                assertEquals(d.plusDays(2), range[1]);
        }

        @Test
        @DisplayName("Second assignment in a series starts the day after the first effective end when ranges overlap")
        void seriesSecondAssignmentStartsAfterFirstEffectiveEnd() {
                ...
                assertEquals(e1.plusDays(1), rangeB[0]);
                assertEquals(e2.plusDays(1), rangeB[1]);
        }

        @Test
        @DisplayName("Assignments without a series do not chain even within the same course")
